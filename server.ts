...
import { users, solvedProblems, expeditionHunts, greedyChats } from './src/db/schema.ts';
import { fetchCodeforcesUserInfo, generateHuntProblems, CFProblem } from './src/server/codeforces.ts';
import { generateGreedyResponse, ChatMessage, ProblemContext } from './src/server/greedyAi.ts';
...
async function startServer() {
  ...
  // Update nice name
  app.post('/api/user/update-nicename', async (req, res) => {
    try {
  ...
  app.get('/api/user/sync-rating/:handle', async (req, res) => {
    try {
  ...
  // --- SOLVED LIBRARY ---
  app.get('/api/solved/:handle', async (req, res) => {
    try {
  ...
  app.post('/api/solved/toggle', async (req, res) => {
    try {
  ...
  app.post('/api/solved/import', async (req, res) => {
    try {
  ...
  // --- EXPEDITIONS ---
  app.get('/api/expedition/:handle', async (req, res) => {
    try {
      const normalizedHandle = req.params.handle.trim().toLowerCase();
      const existing = await db
        .select()
  ...
  // Generate Hunt
  app.post('/api/expedition/generate', async (req, res) => {
    try {
  ...
  app.post('/api/expedition/toggle-problem', async (req, res) => {
    try {
  ...
  app.get('/api/chat/history/:handle/:problemId', async (req, res) => {
    try {
      const normalizedHandle = req.params.handle.trim().toLowerCase();
  ...
  app.post('/api/chat/send', async (req, res) => {
    try {
      ...
      // Generate AI response via Gemini 3.1 Pro (with fallback)
